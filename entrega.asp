...
<body>
<form id="form1" name="form1" method="post" action="tespoyo.asp" onSubmit = "">
  ...
  <table width="800" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td><table width="800" border="0" cellspacing="1" cellpadding="1">
        <tr>
          <td width="22%" align="right">Factura</td>
          <td width="78%"><label>
          
          <br /><%
		  
		  
		   SQL = "Select * from `combustible` where `fe` like '%2010' order by `factura` DESC"
...
Set rs = conn.Execute(SQL)
		  ...
		  %>
		  <%response.Write(rs("factura"))%>--<%if rs("depto") = "" then%>
		  Autorizado en los ticket por el presidente
		  <%else%>
