...
<div align="center">
  ...
  <p>Reporte de kilometros recorridos de los ultimos 10 vales de combustible
    <%
xxo = request.querystring("auto")


' SQL = "Sele'ct * from `combustible` where  `fe` like '%12/2008'  order by `vehiculo`,`id` desc"
SQL = "Select * from `combustible` where `vehiculo` LIKE '%"&xxo&"%'  order by `id` desc limit 30"
'SQL = "Select * from `combustible` where `vehiculo` LIKE '%"&xxo&"%'  order by `id` desc"
